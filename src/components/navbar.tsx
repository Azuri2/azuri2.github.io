...
export default function Navbar() {
  ...
  return (
    <nav className="h-[70px] fixed top-0 left-0 w-full mt-2 bg-black/40 backdrop-blur-md px-6 md:px-16 lg:px-24 xl:px-32 flex items-center justify-between z-50 text-gray-400 border-b border-white/[0.05]">
        ...
        {/* Logo (texte) */}
        <a href="/" className="font-bold tracking-wider text-white text-xl" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>
        ...
        <div className="hidden md:flex items-center gap-8 text-s font-mono tracking-wide">
            <a 
              href="/" 
              className={`transition-colors ${!isHomePage ? 'text-white font-medium hover:text-blue-300' : 'text-blue-400 font-medium'}`}
            >
              Home
            </a>
            <a 
              href="/projects" 
              className={`transition-colors ${!isProjectsPage ? 'text-white font-medium hover:text-blue-300' : 'text-blue-400 font-medium'}`}
            >
              Projects & Skills
            </a>
            <a 
              href="/journey" 
              className={`transition-colors ${!isJourneyPage ? 'text-white font-medium hover:text-blue-300' : 'text-blue-400 font-medium'}`}
            >
              Learning Journey
            </a>
            <a 
        ...
        <button 
          ...
          className="inline-block md:hidden active:scale-90 transition z-50 text-white"
        >
